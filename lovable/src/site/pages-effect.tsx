...
import { ABONEMENTS, IPR_UNIT, CASES, FAQ_ITEMS } from "./data";
import { CaseCard, Contact, FaqAccordion } from "./blocks";

/* ------------------------------ Адреса и PDF ------------------------------ */

/* Адреса страниц раздела — ключи маршрутов (без слеша, как path в ROUTES).
   Для ссылок берите BE_LINK: со слешем на конце, чтобы не ловить 301. */
export const BE = {
...
/* Ответы из /faq, относящиеся к этой услуге: читатель получает их здесь,
   не уходя на отдельную страницу (ревизия 17.09.2026). Тексты — те же
   FAQ_ITEMS, по индексам. */
function FaqPick({ n, pick }: { n: string; pick: number[] }) {
  const items = pick.map((i) => FAQ_ITEMS[i]).filter(Boolean);
  return (
    <section className="stage border-b border-[color:var(--color-line)]">
      <Section n={n} label="Частые вопросы">
        <div className="-mt-8">
          <FaqAccordion items={items} />
        </div>
        <div className="mt-8">
          <a href="/faq/" className="link-arrow group t-body">
...
export function InternalExpertsPage() {
  ...
  const formats: [string, string][] = [
    ...
    ["Нужна подсказка в процессе работы", "схемы, чек-листы, пошаговые алгоритмы и деревья решений"],
  ...
  return (
    <PageShell path={BE.internal}>
        ...
        <PageHead
          ...
          actions={
            <>
              ...
              <QuietLink href={BE_LINK.internalEffect}>Экономический эффект БЕЗ ВОДЫ</QuietLink>
            </>
          }
        />

        <Section n="01" label="Тиражирование практик" title="Когда актуально масштабирование практик">
          <TitledList items={when} />
        </Section>
      </section>

      <section className="relative border-b border-[color:var(--color-line)] bg-[color:var(--color-bg-primary)]">
        <Section n="02" label="Тиражирование практик" title="Систематизация опыта: от носителя знаний в процессы всей компании">
          ...
          <p className="mt-8 max-w-3xl t-body text-[color:var(--color-text-secondary)]">
            Если эти знания остаются неформализованными, компания оказывается зависимой от конкретных лиц. Задача — перевести багаж знаний экспертов в отчуждаемые рабочие активы:
          </p>
          <div className="mt-6 max-w-3xl">
      ...
      <section className="relative border-b border-[color:var(--color-line)] bg-[color:var(--color-bg-primary)]">
        <Section n="04" label="Тиражирование практик" title="Этапы формализации опыта">
          <Steps
      ...
      <section className="relative border-b border-[color:var(--color-line)] bg-[color:var(--color-bg-primary)]">
        <Section n="05" label="Тиражирование практик" title="Форматы итоговых образовательных программ и рабочих артефактов">
          <p className="max-w-3xl t-body text-[color:var(--color-text-secondary)]">
      ...
      <section className="stage sec-dark grain relative border-b border-[color:var(--color-line-dark)]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 sec-pad md:px-8">
          ...
          <p className="mt-6 max-w-3xl t-body text-[color:var(--color-text-inverse-2)]">
            Результат: готовая карта компетенций, которая позволит разрабатывать материалы силами БЕЗ ВОДЫ, передать задачу внутреннему отделу T&D или стороннему подрядчику. Все наработки остаются в собственности клиента
          </p>
          ...
          <div className="mt-8 flex flex-col items-start gap-4 sm:flex-row sm:flex-wrap sm:items-center sm:gap-6">
            ...
            <QuietLink href={BE_LINK.internalEffect}>Экономический эффект</QuietLink>
          </div>
        </div>
      </section>

      <FaqPick n="06" pick={[2, 9, 11, 13]} />
      <Contact />
    </PageShell>
  );
}
...
export function InternalExpertsEffectPage() {
  const effects = [
    {
      title: "Высвобождение до 30–40% рабочего времени ключевых сотрудников*",
  ...
  const risks: [string, string][] = [
    ["Прозрачные критерии приемки до старта работ", "Состав артефактов (алгоритмы, тренажеры, база знаний) и требования к ним фиксируются в договоре до начала реализации."],
    ["Единая точка контроля и ответственности", "Проектное бюро БЕЗ ВОДЫ берет на себя всю методологическую часть, работу с экспертами и подготовку учебных форматов по одному рамочному договору"],
    ["Поэтапный контроль инвестиций: Карта экспертности (7–14 дней)", "Первый шаг — разработка Карты экспертности, которая является самостоятельным законченным продуктом. Результат: карта знаний, карта процесса и решений, матрица компетенций, форматы, архитектура базы знаний и дорожная карта"],
  ];
  ...
  return (
    <PageShell path={BE.internalEffect}>
        ...
        <PageHead
          ...
          actions={
            <>
              ...
              <QuietLink href={BE_LINK.internal}>Как построена передача опыта ключевых сотрудников</QuietLink>
            </>
          }
        />
        <div className="relative mx-auto max-w-7xl px-5 sec-pad md:px-8">
      ...
      <section className="relative border-b border-[color:var(--color-line)] bg-[color:var(--color-bg-primary)]">
        <Section n="01" label="Тиражирование практик" title="Сравнение операционных моделей">
          <Table3
            ...
            rows={[
              ...
              ["Стоимость масштабирования", "Линейно растет нагрузка на экспертов и руководителей", "Единовременная инвестиция в разработку с бесплатным тиражированием"],
      ...
      <section className="relative border-b border-[color:var(--color-line)] bg-[color:var(--color-bg-primary)]">
        <Section n="02" label="Тиражирование практик" title="Снижение управленческих и финансовых рисков проекта">
          <TitledCards items={risks} />
          <p className="mt-8 max-w-3xl t-caption text-[color:var(--color-text-secondary)]">
            * Расчетные значения основаны на проектах БЕЗ ВОДЫ; показатели кейсов — по данным заказчиков. Не являются гарантией результата.
          </p>
        </Section>
      </section>
...
const SUB_GROUPS: { title: string; items: Unit[] }[] = [
  {
    ...
    items: [
      ...
      ["Регулярное обучение по разработанной программе", "40 часов обучения по согласованному расписанию, по разработанным материалам"],
  ...
  },
  {
    ...
    items: [
      ["Онлайн-курс или сценарий тренинга — срок 10 дней", "полный комплект материалов: паспорт проекта, сценарий, раздаточные материалы, презентация, программа, дополнительные материалы для самостоятельного чтения и т. д."],
      ["Адаптация материалов — оплата за единицу, срок 2 дня", "внесение правок в готовые материалы под новую аудиторию или формат (онлайн или офлайн, обновления и другие изменения)"],
      ["Подготовка курса в LMS за 2 дня", "готовый материал оформлен и опубликован в системе LMS"],
      ["Подписка на поддержание актуальности разработанных материалов на платформе, благодаря чему они не устаревают"],
    ],
  },
  {
    ...
    items: [
      ...
      ["Цифровой наставник по базе знаний за 10 дней", "подскажет, сформулирует, структурирует, предложит — поддержит команду круглосуточно"],
    ],
  },
  {
    title: "ИИ-автоматизация и сопровождение",
    items: [
      ["Автоматизация процессов обучения «под ключ»", "надстройка на действующие сервисы компании, без замены системы"],
...
export function TeamSubscriptionPage() {
  const corp: [string, string[]][] = [
    ...
    ["Эластичность ресурсов с учетом неравномерной нагрузки", [
      ...
      "Возможность быстро подключить готовую команду на полгода, закрыть пик задач и остановить работы, когда они больше не нужны.",
    ]],
    ["Преодоление внутренней бюрократии и скорости найма", [
      ...
      "Подписка на услуги оформляется как сервисный контракт (B2B-услуга, OpEx), бюджет на который часто уже запланирован или согласуется быстрее, чем расширение штатного расписания. Старт через 24 часа.",
    ]],
  ];
  const edtech: [string, string[]][] = [
    ["Пропускная мощность отдела (Устранение узкого горлышка)", [
      "Roadmap продуктовой команды, как правило, расписан на год вперед. Внезапно появляется тренд (например, внедрение ИИ), и курс нужно выпустить в оборот через месяц, иначе преимущество будет у конкурентов. Внутренние продюсеры перегружены текущими проектами. Команда подключается как параллельный конвейер, который позволяет выпустить продукт в срок, не ломая текущий план разработки.",
    ]],
    ["Управление экономикой продукта (Unit-экономика)", [
      "Наем людей в штат повышает постоянные издержки бизнеса. Оплачивая разработку по подписке, CPO переводит затраты на разработку в переменную часть (Cost of Goods Sold).",
    ]],
  ...
  const principle: [string, string][] = [
    ...
    ["Старт за 24 часа", "подключение проектной команды (ведущий методолог, проджект-менеджер, сборщик курсов) без необходимости проходить весь цикл подбора и адаптации."],
  ...
  const comfort = [
    "Под каждую задачу будут необходимые ресурсы: методология, разработка и управление проектом.",
    ...
    "Если возникают сложности с исполнителями или доступами к инструментам, вопрос решается в течение 2 суток, и на сроках и качестве это не отражается.",
  ];
    ...
    <PageShell path={BE.team}>
      <section className="stage border-b border-[color:var(--color-line)]">
        ...
        <PageHead
          ...
          lead="Проектная команда для отделов обучения и EdTech-компаний. Решение задач по разработке образовательных программ, реализации тренингов и передаче готового продукта в распоряжение клиента без увеличения постоянного штата."
          actions={
            <>
              <FormButton>Оставить заявку на расчет подписки</FormButton>
              <QuietLink href={BE_LINK.teamEffect}>Экономический эффект</QuietLink>
            </>
          }
        />

        <Section n="01" label="Подписка на отдел обучения" title="Какие задачи решает подписка">
          <div className="space-y-12">
            ...
            <div>
              ...
              <p className="mt-4 max-w-3xl t-body text-[color:var(--color-text-secondary)]">
                Чтобы проверить спрос на новую тему или профессию, создавать полноценный 6-месячный курс со своей штатной командой слишком долго и рискованно. Команда за 24 часа собирает группу для производства MVP-продукта (интенсив, микрокурс, серия воркшопов, интерактивный тренажер или марафон), и через 2–3 недели можно испытать новую разработку. EdTech тестирует конверсию, собирает обратную связь от первой когорты и принимает решение о запуске нового курса или программы без заморозки внутренних ресурсов.
              </p>
            </div>
          </div>
        </Section>
      ...
      <section className="relative border-b border-[color:var(--color-line)] bg-[color:var(--color-bg-primary)]">
        <Section n="02" label="Подписка на отдел обучения" title="Принцип работы: результат вместо человеко-часов">
      ...
      <section className="stage sec-dark grain relative border-b border-[color:var(--color-line-dark)]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 sec-pad md:px-8">
          <SectionLabel n="04">Подписка</SectionLabel>
        ...
        <div className="relative mx-auto max-w-7xl px-5 sec-pad md:px-8">
          ...
          <div className="max-w-3xl">
            <RevealHeading className="t-h2 mt-6 text-[color:var(--color-text-inverse)]">
              Прозрачность данных по подписке
            </RevealHeading>
            <p className="mt-6 t-body text-[color:var(--color-text-inverse-2)]">
              Состав и план работ на месяц фиксируются заранее и видны в личном кабинете: что заказано, что в работе, что принято, каков остаток средств по каждому пакету. Движение показано в единицах результата, а не в часах. При изменении состава работ остаток сразу же пересчитывается.
            </p>
          </div>
        </div>
      </section>

      <FaqPick n="06" pick={[0, 1, 8, 12]} />
      <Contact />
    </PageShell>
  );
}
...
export function TeamSubscriptionEffectPage() {
  const manage: [string, string][] = [
    ["Взаимозаменяемость единиц результата", "Внутри оплаченного пакета можно перераспределять задачи"],
    ["Прозрачность в личном кабинете", "Контроль ведется по статусам выполнения артефактов, а не по отчетам о трудозатратах"],
    ["Опция поддержания актуальности", "После завершения крупных релизов подписка переводится в режим поддержки на минимальный объем, чтобы материалы своевременно обновлялись"],
  ];
  return (
    <PageShell path={BE.teamEffect}>
        ...
        <PageHead
          ...
          actions={
            <>
              ...
              <QuietLink href={BE_LINK.team}>Состав услуг</QuietLink>
            </>
          }
        />

        <Section n="01" label="Подписка на отдел обучения" title="Сравним: штатная команда vs подписка на отдел обучения">
          <p className="max-w-3xl t-body text-[color:var(--color-text-secondary)]">
          ...
          <div className="mt-8">
            <Table3
              head={["Статья расходов", "Штатная команда (3 роли)", "Подписка на отдел обучения"]}
              rows={[
                ["Методолог / Старший методист", "160 000–180 000 ₽ / мес. (gross)", "Включен в состав рабочей группы"],
                ["Руководитель проекта (PM)", "200 000–290 000 ₽ / мес. (gross)", "Включен в состав рабочей группы"],
                ["Сборщик курсов / LMS-верстальщик", "110 000–130 000 ₽ / мес. (gross)", "Включен в состав рабочей группы"],
                ["ФОТ в месяц*", "470 000–600 000 ₽", "от 180 000 ₽"],
                ["Налоги и страховые взносы (~30%)", "+ 141 000–180 000 ₽ / мес.", "0 ₽"],
                ["Рекрутинг (агентство/HR)", "от 350 000 ₽ (разово за команду)", "0 ₽"],
                ["Оснащение, ПО, онбординг", "от 150 000 ₽ (разово) + лицензии", "0 ₽"],
                ["Итого реальные затраты (1-й мес.)", "от 1 100 000 ₽", "от 180 000 ₽"],
              ]}
              total={["Ежемесячные затраты со 2-го месяца", "от 600 000 ₽ / мес.", "Фиксированный пакет по договору (от 180 000 ₽)"]}
            />
          </div>
          <p className="mt-5 max-w-3xl t-caption text-[color:var(--color-text-secondary)]">
            * Расчет фонда оплаты труда основан на медианных зарплатных предложениях (hh.ru, Москва, выборка на август 2026 года).
          </p>
        </Section>
      </section>
          ...
          <div className="grid items-stretch gap-4 md:grid-cols-2">
              ...
              <div className="mt-4">
                <NodeList
                  items={[
                    ...
                    "Нужен разовый доступ к редкой экспертизе высокого уровня (знания специалистов по сложным техническим темам, проектирование ИИ-агентов)",
                  ]}
                />
              </div>
            </PaperCard>
            <PaperCard className="h-full p-6">
              ...
              <div className="mt-4">
                <NodeList
                  items={[
                    "Если объем задач по обучению однотипен, полностью стабилизирован и предсказуем на 2–3 года вперед — дешевле выстраивать внутреннюю операционную команду.",
                  ]}
                />
              </div>
            </PaperCard>
      ...
      <section className="relative border-b border-[color:var(--color-line)] bg-[color:var(--color-bg-primary)]">
        <Section n="03" label="Подписка на отдел обучения">
          <TitledCards items={manage} />
          <p className="mt-8 max-w-3xl t-body text-[color:var(--color-text-secondary)]">
            Подписка — услуга с результатом (программы, тренинги, материалы), а не сотрудники в штате заказчика: состав, график и методы работы определяет исполнитель. Работаем по договору возмездного оказания услуг с ИП Уткина В. В.
          </p>
        </Section>
      </section>
...
export function ExternalExpertsPage() {
  const steps = [
    "30 минут — обсуждение задачи и определение, какой именно опыт здесь необходим и по каким признакам поймем, что результат достигнут",
    "В течение 60 минут после обсуждения задачи — описание опыта и подтвержденные результаты практиков в нужной области.",
    "Знакомство с экспертом, чей опыт ляжет в основу решения",
    "Эксперт участвует в разработке решения. Если для нужного результата требуется иное видение или подход — привлекаем другого эксперта",
    "Фиксация логики решений эксперта и перенос в обучающие материалы.",
  ];
  return (
    <PageShell path={BE.external}>
      <section className="stage border-b border-[color:var(--color-line)]">
        <Scene blobs={[{ className: "-right-40 top-[20%]", tone: "rose", size: 520 }]} />
        <PageHead
          kicker="Привлечение внешних экспертов"
          title={<>Практики, опробованные в реальных условиях</>}
          lead="Знания и навыки, которыми не владеют специалисты внутри компании, можно получить без долгого поиска и обращений к консалтинговым агентствам."
          actions={
            <>
              <FormButton>Оставить заявку на разбор задачи</FormButton>
              <QuietLink href={BE_LINK.externalEffect}>Экономический эффект</QuietLink>
            </>
          }
        />
        <Section n="01" label="Привлечение внешних экспертов">
          <Steps items={steps} />
        </Section>
      </section>

      <FaqPick n="02" pick={[3, 5, 6]} />
      <Contact />
    </PageShell>
  );
}
...
export function ExternalExpertsEffectPage() {
  const groups: [string, [string, string][]][] = [
    ["Сокращение сроков запуска (Time-to-Market)", [
      ["Старт проекта за 24 часа вместо 1–2 месяцев", "подключение проверенного эксперта под конкретную задачу без необходимости запускать стандартный цикл поиска"],
    ...
    ["Сохранение и защита знаний внутри компании", [
      ["Нет зависимости от внешних экспертов", "методология и логика решений эксперта документируются и передаются в собственность компании (регламенты, база знаний, обучающие модули)"],
    ...
    <PageShell path={BE.externalEffect}>
        ...
        <PageHead
          ...
          actions={
            <>
              ...
              <QuietLink href={BE_LINK.external}>Как подключаем внешних экспертов</QuietLink>
            </>
          }
        />
      </section>
      ...
      {groups.map(([t, items], i) => (
        <section key={t} className="relative border-b border-[color:var(--color-line)] bg-[color:var(--color-bg-primary)]">
          <Section n={String(i + 1).padStart(2, "0")} label="Привлечение внешних экспертов" title={t}>
...
export function BusinessEffectGeneralPage() {
  /* Редакция 17.09.2026 по предложению, согласованному Викторией («все ок»):
     только проверяемое — цифры кейсов (по данным заказчиков), таблица ФОТ
     (hh.ru), условия оферты и FAQ; расчетные значения — со сноской. Снято:
     «до 40%», «×2,5», «≈ 3 FTE», «> 70%», «> 2 млн», «до 0%», «≤ 18 мес»,
     «±5%», CapEx/OpEx, FTE, TTV, RFI/RFP, C-level. */
  const groups: [string, [string, string?][]][] = [
    ["Материалы остаются у заказчика", [
      ["100% прав", "исключительные права на программы, сценарии, тренажеры и исходные файлы переходят заказчику после подписания акта и оплаты. Без лицензий, подписок и роялти на созданные материалы"],
      ["Без привязки к платформе", "исходники в открытых редактируемых форматах, LMS можно менять без потери контента"],
      ["Сопровождение — по желанию", "после сдачи проекта обязательных платежей нет; поддержание актуальности материалов — отдельная услуга по подписке"],
    ]],
    ["Дешевле штатной команды, оплата за результат", [
      ["Без найма проектной команды", "методолог, руководитель проекта и сборщик курсов в штате обходятся от 600 000 ₽ в месяц (ФОТ по hh.ru, Москва, август 2026, с налогами и взносами); подписка на отдел обучения — от 180 000 ₽ в месяц"],
      ["Оплата за готовые артефакты", "программа, тренажер, регламент, проведенная сессия. Не за отработанные часы, созвоны и внутренние согласования"],
      ["Бюджет фиксируется в смете", "изменения состава и стоимости — только по согласованию сторон"],
      ["Услуга, а не штат", "расходы идут как сервисный контракт, бюджет на который согласуется быстрее, чем расширение штатного расписания"],
    ]],
    ["Быстрее, чем нанимать и согласовывать", [
      ["Старт через 24 часа", "без тендера и онбординга, который при найме занимает 1–3 месяца"],
      ["Параллельная разработка", "несколько экспертов работают над разными модулями одновременно: программа из 10 модулей — 4–6 недель вместо 3 месяцев*"],
      ["Время эксперта заказчика — 4–6 часов на весь проект", "плюс 30 минут на финальную проверку материалов"],
      ["Перерасчет остатка по подписке — за 1 рабочий день", "без перезапуска договора"],
    ]],
  ];
  const why = [
    "Только обучение: извлекаем скрытый практический опыт экспертов и инженеров и превращаем их подходы в понятные сотрудникам алгоритмы",
    "Материалы переиспользуются: созданное встраивается в базу знаний и используется командой заказчика без внешних подрядчиков",
    "Один договор вместо десятка договоров с фрилансерами и координации между ними",
    "Заказчик проверяет готовые смыслы, а не пишет материалы: 30–40 минут на проверку вместо недель подготовки текстов",
    "Эксперты с многолетней практикой: команда сразу решает задачу, этапа обучения за счет клиента нет",
    "Проект закрыт только после пилота: программа считается сданной, когда пилотная группа решила реальную задачу по новому стандарту",
  ];
  ...
  return (
    <PageShell path={BE.general}>
      <section className="stage border-b border-[color:var(--color-line)]">
        <Scene blobs={[{ className: "-right-40 top-[20%]", tone: "rose", size: 520 }]} />
        <PageHead
          kicker="Экономический эффект"
          title={<>Экономический эффект: ROI обучения</>}
          chips={[
            ["в 6 раз", "Ускорение адаптации новых сотрудников: федеральная ювелирная сеть, >230 магазинов, выход на KPI за 1 месяц"],
            ["−16%", "Оттока после перестройки предложения: B2B-компания, эксперт вышел в проект через 72 часа после запроса"],
          ]}
          actions={
            <>
              <FormButton>Оставить заявку на разбор задачи</FormButton>
      ...
      <section className="stage sec-dark grain relative border-b border-[color:var(--color-line-dark)]">
        <Scene blobs={[{ className: "-left-40 top-0", tone: "chrome", size: 420 }]} />
        <div className="relative mx-auto max-w-7xl px-5 sec-pad md:px-8">
          <SectionLabel n="04">Почему БЕЗ ВОДЫ / специализация</SectionLabel>
          <p className="mt-6 max-w-3xl t-body text-[color:var(--color-text-inverse)]">
            БЕЗ ВОДЫ — не консалтинг широкого профиля и не кадровое агентство. Специализация команды — промышленная разработка учебного дизайна и проектное управление знаниями для корпоративных университетов и EdTech-компаний.
          </p>
          ...
          <p className="mt-6 max-w-3xl t-caption text-[color:var(--color-text-inverse-2)]">
            * Расчетные значения основаны на проектах БЕЗ ВОДЫ; показатели кейсов — по данным заказчиков. Не являются гарантией результата.
          </p>
        </div>
      </section>
