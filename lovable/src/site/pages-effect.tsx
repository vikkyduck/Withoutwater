...
export function ExternalExpertsPage() {
  const steps = [
    ...
    "В течение 60 минут после обсуждения задачи — описание опыта и подтвержденные кейсы практиков, которые будут работать над задачей в рамках проекта.",
    "Знакомство с практиком и согласование состава команды под задачу",
    "Согласованный эксперт работает в проекте. Меняем состав команды, если понимаем, что для нужного результата требуется иное видение или подход",
  ...
  return (
    <PageShell path={BE.external}>
      <section className="stage border-b border-[color:var(--color-line)]">
        ...
        />
        <Section n="01" label="Внешние эксперты">
          <Steps items={steps} />
        </Section>
      </section>
