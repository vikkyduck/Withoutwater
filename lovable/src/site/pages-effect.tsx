...
export function InternalExpertsEffectPage() {
  const effects = [
    {
      title: "Высвобождение до 30–40% рабочего времени сотрудников обучения",
  ...
  const risks: [string, string][] = [
    ...
    ["Поэтапный контроль инвестиций: Карта экспертности (7–14 дней)", "Первый шаг — разработка Карты экспертности, которая является самостоятельным законченным продуктом. Результат: систематизированная карта знаний, матрица компетенций и дерево решений"],
  ];
        ...
        <PageHead
          ...
          actions={
            <>
              ...
              <QuietLink href={BE_LINK.external}>Стандарт привлечения практиков</QuietLink>
            </>
          }
        />
      </section>
...
export function BusinessEffectGeneralPage() {
  const groups: [string, [string, string?][]][] = [
    ...
    ["Оптимизация бюджета T&D и операционная гибкость", [
      ...
      ["Прозрачный CapEx вместо OpEx", "материальный актив сразу, а не распределенные расходы по году"],
    ]],
    ["Сокращение сроков запуска программ (Time-to-Delivery)", [
      ["Старт через 24 часа", "подключение рабочей группы без RFI/RFP, тендеров и онбординга, который обычно занимает 1–3 месяца"],
  ...
  const why = [
    ...
    "13+ лет — средний опыт экспертов. Никакого этапа обучения за счет клиента: команда сразу решает задачу",
  ...
  return (
    <PageShell path={BE.general}>
      <section className="stage border-b border-[color:var(--color-line)]">
        <Scene blobs={[{ className: "-right-40 top-[20%]", tone: "rose", size: 520 }]} />
        <PageHead
          kicker="Экономический эффект от услуг БЕЗ ВОДЫ"
      ...
      <section className="stage sec-dark grain relative border-b border-[color:var(--color-line-dark)]">
        ...
        <div className="relative mx-auto max-w-7xl px-5 sec-pad md:px-8">
          ...
          <p className="mt-6 max-w-3xl t-body text-[color:var(--color-text-inverse)]">
            БЕЗ ВОДЫ — не консалтинг широкого профиля и не кадровое агентство. Специальность команды — промышленная разработка учебного дизайна и проектное управление знаниями для корпоративных университетов и EdTech-компаний.
          </p>
