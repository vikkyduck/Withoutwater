...
Перед каждой задачей обязательно прочитать `AGENTS.md`, `docs/PROJECT-CONTEXT.md` и `docs/CURRENT-STATE.md`. Они фиксируют актуальные решения владельца и границу test/production. Противоречащие им исторические указания этого файла не применять; в частности, редактирование тестовой версии не разрешает запуск deploy.sh.
